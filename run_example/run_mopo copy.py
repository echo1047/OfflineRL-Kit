...
def get_args():
    parser = argparse.ArgumentParser(description='introduction to mopo')
    ...
    parser.add_argument("--auto-alpha", default=True, help="automatically tune alpha") #? alpha是什么参?
...
def train(args=get_args()):
    ...
    # create policy model
    actor_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims) # return a model
    ...
    actor = ActorProb(actor_backbone, dist, args.device) # actor_backbone + dist_net
    critic1 = Critic(critic1_backbone, args.device) # critic1_backbone + last(Linear)
    critic2 = Critic(critic2_backbone, args.device)
    actor_optim = torch.optim.Adam(actor.parameters(), lr=args.actor_lr)
    critic1_optim = torch.optim.Adam(critic1.parameters(), lr=args.critic_lr)
    critic2_optim = torch.optim.Adam(critic2.parameters(), lr=args.critic_lr)
    '''
    lr_scheduler 是一个学习率调度器的对象，它可以根据一定的规则动态地调整优化器的学习率。
    torch.optim.lr_scheduler.CosineAnnealingLR 是一个使用余弦退火策略的学习率调度器，它可以让学习率在每个周期内先下降后上升，模拟一个重启的过程。
    actor_optim 是一个优化器的对象，它可以用来更新模型的参数。
    args.epoch 是一个整数，表示训练的总轮数。
    这段代码的作用是创建一个学习率调度器，它会在每个周期内根据余弦函数的形式调整actor_optim的学习率，周期的长度为args.epoch。
    '''
